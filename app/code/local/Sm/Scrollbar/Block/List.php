...
class Sm_Scrollbar_Block_List extends Mage_Catalog_Block_Product_Abstract
{
	...
	public function __construct($attributes = array()){
		parent::__construct();
		$this->_config = Mage::helper('scrollbar/data')->get($attributes);
	}
	
	public function getConfig($name=null, $value=null){
		if (is_null($this->_config)){
			$this->_config = Mage::helper('scrollbar/data')->get(null);
		}
	...
	protected function _toHtml(){
		...
		$template_file = "sm/scrollbar/default.phtml";
	...
	protected function _getItems(){
		...
		$collect_items = array();
		$this->addReviewSummaryTemplate('sm', 'sm/scrollbar/summary.phtml');
		foreach( $products as $k => $_product ) {
			$item['id'] = $_product->getId();
			$item['title'] =  Mage::helper('scrollbar/data')->truncate($_product->getName(), $this->_config['product_title_max_characters'], '...');
			...
			if (($maxchars=$this->getConfig('product_description_max_characters',-1))>0){
				$item['desc'] =  Mage::helper('scrollbar/data')->truncate($item['desc'], $maxchars, '...');
			}
	...
	public function getScriptTags(){
		...
		if (null == Mage::registry('jsmart.jquery')){
			...
			if (Mage::getStoreConfigFlag('scrollbar_cfg/advanced/include_jquery')){
				// if module allowed jquery.
				$import_str .= $jsHelper->includeSkinScript('sm/scrollbar/js/jquery-1.8.2.min.js');
				Mage::register('jsmart.jquery', 1);
			}
		}
		if (null == Mage::registry('jsmart.jquerynoconfict')){
			// add once noConflict
			$import_str .= $jsHelper->includeSkinScript('sm/scrollbar/js/jquery-noconflict.js');
			Mage::register('jsmart.jquerynoconfict', 1);
		}
		if (null == Mage::registry('jsmart.scrollbar.js')){
			// add script for this module.
			$import_str .= $jsHelper->includeSkinScript('sm/scrollbar/js/jquery.mCustomScrollbar.concat.min.js');
			Mage::register('jsmart.scrollbar.js', 1);
		}
		return $import_str;
	}
